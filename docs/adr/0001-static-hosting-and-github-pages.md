...
The site must be a low-cost, low-maintenance credibility artifact for a stealth, pre-launch protocol. The FW.VISION constellation deploys its sites on GitHub Pages. The contact page needs a way for visitors to reach out.
...
2. **No server-side form.** GitHub Pages is static; there is no backend. Rather than route visitor data through a third-party form processor (Formspree, EmailJS, etc.) - a poor fit for a stealth, sovereignty-focused brand - the contact page presents an **illustrative disabled template** plus a **`mailto:hello@daicompute.ca`** button prefilled with a structured template. This is first-party (the visitor's own mail client), no third party touches the data.
...
- If a real submission pipeline is wanted later, prefer a **first-party self-hosted handler** (e.g. on the Linux box that runs the CITAble app) over a third-party processor, to preserve the sovereignty/stealth posture.
